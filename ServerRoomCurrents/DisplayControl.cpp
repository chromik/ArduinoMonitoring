...
extern struct str_display display_settings; // nastaveni displeje (jas, kontrast)
...
void control_proud(int stisknuto) {
...
void control_menu(int stisknuto) { 
  ...
  if (stisknuto == BT_SELECT) { // vyber polozky v menu
    switch (scr2render->cislo) {
      case 0: // Nastaveni proudovech limitu
        scr2render = create_screen_proudove_limity_glob();
        break;
      case 1: // Nataveni LCD
      ...
      case 5: // ZAVRIT MENU
        ...
        scr2render = scr2render->menu; // prejdu na obrazovku proudu (menu odkazuje na obrazovky proudu);
        break;
    }
  }
}
...
// kontrola menu pro nastavovani casu
...
void control_proudove_limity_glob(int stisknuto) {
  ...
  if (scr2render->cislo == 0) {
    ...
    if (stisknuto == BT_LEFT) {
      if ((scr2render->property1 - 1) > scr2render->property2 ) {
        scr2render->property1 -= 0.25;
      }
    }

    // ZVYSOVANI MAX LIMITU
    else if (stisknuto == BT_RIGHT) {
      if (scr2render->property1 < SENSOR_MAX_CURR) {
        scr2render->property1 += 0.25;
      }
    }
    ...
    // VRATIT SE DO MENU a neukladat nastavene proudy
    else if (stisknuto == BT_MENU) {
      screen_dealloc_back();
    }
    ...
    if (stisknuto == BT_LEFT) {
      if (scr2render->property2 > SENSOR_MIN_CURR) {
    ...
    if (stisknuto == BT_LEFT) {
      ...
      if (scr2render->property2 < scr2render->property1) {
        save_currlimit_global(scr2render->property2, scr2render->property1, scr2render->prop[0]);
        screen_dealloc_back();
      }
...
void button_controlling(void) {
  ...
  switch (scr2render->typ) {
    case SCR_MENU:
      ...
      break;
    case SCR_CURR:
      control_proud(stisknuto);
      break;
    case SCR_LIMS:
      control_proudove_limity_glob(stisknuto);
      break;
    case SCR_LCDSET:
