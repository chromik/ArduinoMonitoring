...
int sendmail_via_smtp(String to, String subject, String text) {
  ...
  if (ethernet.client->connect(email.smtp_server, email.smtp_port) < 0) {
    ...
    display.display();
    #if BAS_DEBUG
      Serial.println(F("Failed - connection problem."));
    #endif
    return 0;
  }
  ...
  ethernet.client->println(ethernet.public_ip); // HELO po navazani spojeni s SMTP (public IP je pevne definovana v ServerTemperatures.ino; pokud server neoveruje IP, muze byt libovolny text)
  ...
  // MAIL FROM
  PROGMEM const char * str_from = "MAIL From: <";
  ethernet.client->print(str_from);
  ...
  // RCPT TO
  PROGMEM const char * str_to = "RCPT To: <";
  ethernet.client->print(str_to);
  ...
  // DATA
  ethernet.client->println(F("DATA"));
  if(!eRcv()) return 0;
  // DATA
  ...
  // Predmet emailu
  ethernet.client->print(F("Subject: "));
  ethernet.client->println(subject);

  // Prijemce emailu
  PROGMEM const char * str_to2 = "To: You <";
  ethernet.client->print(str_to2);
  ethernet.client->print(to);
  ethernet.client->println(">");
  // To: You <xxx@xxx.xx>

  // Odesilatel emailu
  PROGMEM const char * str_from2 = "From: TANGER Arduino <";
  ethernet.client->print(str_from2);
  ethernet.client->print(email.from);
  ethernet.client->println(">");
  // From: Me <xxx@xxx.xx>

  // Kodovani emailu
  PROGMEM const char * str_content_type = "Content-Type: text/html; charset=\"UTF-8\"";
  ethernet.client->println(str_content_type);
